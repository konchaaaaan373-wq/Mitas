...
UUID は自動生成で構いません。seed がメールアドレスから `auth.users.id` を自動取得します。

### 1.5 ライブデモシード投入
1. Supabase SQL Editor で `db/seeds/0003_live_demo_seed.sql` をそのまま実行（UUID 置換不要）
2. 出力に `[Mitas LIVE seed] auth.users 解決完了: ...` と `Mitas for Alliance ライブデモシード投入完了（フィクション）` が出ること
3. `[Mitas LIVE seed] 以下の Auth ユーザーが auth.users に見つかりません: ...` が出た場合は、メッセージに列挙されたメールを 1.4 で再作成
   （EXCEPTION でロールバックされるので、再実行で問題なく再開できます）
...
### 5.2 Auth ユーザー削除
Supabase ダッシュボード → Authentication → ユーザー一覧から `konchaaaaan373+mitas-*@gmail.com` を個別削除。
